...
export default function RegisterOtpScreen() {
  ...
  const onResend = async () => {
    ...
    try {
      ...
      Alert.alert(
        "E-mail",
        "Un nouveau code a été envoyé si l'adresse est éligible.",
      );
    } catch (e) {
      setRootMessage(
  ...
  return (
    ...
    >
      <View className="mb-10 mt-5 rounded-[24px] border border-[#D1D5DB] bg-white px-5 pb-7 pt-8">
        <View className="items-center">
          ...
          <Text className="mt-4 text-center text-[14px] font-normal leading-none text-[#52525B]">
            Un code de confirmation a été envoyé à
          </Text>
          <Text className="mb-4 mt-1 text-center text-[14px] font-semibold leading-none">
            {email || "—"}
          </Text>
        </View>
        ...
        {errors.code ? (
          <Text className="mt-2 text-center font-sans text-caption text-destructive">
            {errors.code.message}
          </Text>
        ) : null}
        {rootMessage ? (
        ...
        <View className="mt-4 flex-row items-center justify-center">
          <Text className="text-badge font-normal text-[#52525B]">
            vous n&apos;avez pas reçu l&apos;email ?{" "}
          </Text>
          <Pressable disabled={!email} onPress={() => void onResend()}>
            <Text className="text-badge font-semibold">Renvoyer</Text>
          </Pressable>
        </View>
        ...
        <View className="mt-8 border-t border-[#E5E7EB] pt-5">
          <View className="flex-row items-start pr-12">
            ...
            <Text className="text-center text-badge font-normal leading-normal tracking-normal text-[#737373]">
